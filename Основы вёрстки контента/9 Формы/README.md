...
Текстовые поля формы лежат в обёртке с классом .form-input. Отступ снизу от каждого поля создаётся с помощью класса .mb-2. Для каждого текстового поля обязательно наличие label, который будет скрыт с помощью класса .sr-only. Его реализация уже находится в файле app.css.
